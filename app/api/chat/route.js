...
// ==================================================
// Stores: { ip: { count, firstRequestTime } }
// Resets window every WINDOW_MS
...
function checkRateLimit(ip) {
    ...
    // Cleanup old entries periodically (prevent memory leak for 10-20 users)
...
function sanitizeInput(text) {
    ...
    // Trim, limit length to 2000 chars, remove null bytes
...
export async function POST(request) {
    try {
        ...
        if (!validateMessages(messages)) {
            return Response.json(
                { error: 'Invalid messages format. Each message needs role and content.' },
                { status: 400 }
            );
        }
